...
exports.InsertStatement = (tablename, prefix, columns) => {
  ...
  let cols = columns.map((col) => `${prefix}${col}`).join(",");
...
exports.SelectStatement = (str, data) => {
  if (!Array.isArray(data)) {
    data = [data]; // Convert non-array input to an array
  }
  ...
  for (let i = 0; i < str.length; i++) {
    if (str[i] === "?") {
      if (found < data.length) {
        ...
        throw new Error("Insufficient data provided for placeholders in the string.");
      }
    } else {
      statement += str[i];
    }
...
exports.SelectAllStatementDesc = (tablename, columns, column_name, limit, offset) => {
  let statement = `SELECT ${columns} FROM ${tablename} ORDER BY ${column_name} DESC`;
...
exports.SelectWhereStatementOr = (tablename, columns, condition = [], values = []) => {
  let cols = "";

  for (let i = 0; i < condition.length; i++) {
    cols += `${condition[i]} = '${values[i]}' OR `;
  }
